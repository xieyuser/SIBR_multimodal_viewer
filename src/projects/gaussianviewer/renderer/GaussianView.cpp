...
int loadPly(
    ...
    std::vector<float>& gs_seg,
    std::vector<float>& gs_contri,
    sibr::Vector3f& minn,
    sibr::Vector3f& maxx) {
  ...
  gs_seg.resize(count * 15);
  gs_contri.resize(count * 1);
  ...
  for (int k = 0; k < count; k++) {
    ...
    for (int j = 0; j < 15; j++) {
      gs_seg[15 * k + j] = points[i].seg[j];
    }
    for (int j = 0; j < 1; j++) {
      gs_contri[k + j] = points[i].contri[j];
    }
  }
  return count;
}
...
void savePly(
  ...
  std::string props_contri[] = {"contri"};
  ...
  for (auto s : props_base_scale)
    outfile << "property float " << s << std::endl;
  for (auto s : props_seg)
    outfile << "property float " << s << std::endl;
  for (auto s : props_contri)
    outfile << "property float " << s << std::endl;
  ...
  for (int i = 0; i < pos.size() / 3; i++) {
    ...
    for (int j = 0; j < 15; j++) {
      points[i].seg[j] = seg[15 * i + j];
    }
    for (int j = 0; j < 1; j++) {
      points[i].contri[j] = contri[i + j];
    }
    count++;
  }
...
sibr::GaussianView::GaussianView(
  ...
  std::string contri_mlp_path = plyPath + "contri_mlp.pt";
  std::string shobj_mlp_path = plyPath + "shobj_mlp.pt";
  ...
  SIBR_LOG << "contri_mlp : " << isFileExists_fopen(contri_mlp_path) << std::endl;
  SIBR_LOG << "shobj_mlp : " << isFileExists_fopen(shobj_mlp_path) << std::endl;
  ...
  contri_mlp_module = torch::jit::load(contri_mlp_path, _libtorch_device);
  shobj_mlp_module = torch::jit::load(shobj_mlp_path, _libtorch_device);
  ...
  ak_contri_all = torch::from_blob(anchor_contri.data(), {count, 1}, torch::kFloat32).to(_libtorch_device);
  ak_seg_all = torch::from_blob(anchor_seg.data(), {count, 15}, torch::kFloat32).to(_libtorch_device);
...
void sibr::GaussianView::onRenderIBR(sibr::IRenderTarget& dst, const sibr::Camera& eye) {
  ...
  if (currMode == "Initial Points") {
    ...
    torch::Tensor radii = torch::full({count}, 1, ak_pos_all.options().dtype(torch::kInt32));

    // SIBR_LOG << "ToTal radii" << std::endl;
    if (count != 0) {
      int M = 0;
    ...
    // SIBR_LOG << "ToTal shawansyi" << std::endl;

    radii = (radii > 0).to(_libtorch_device);

    torch::Tensor gs_pos = gs_pos_all.index({radii});
    torch::Tensor ak_contri = ak_contri_all.index({radii});
    torch::Tensor ak_seg = ak_seg_all.index({radii});
    ...
    torch::Tensor gs_color, gs_seg, gs_contri;
    if (_add_appearance) {
      ...
      if (_add_color_dist) {
        ...
        gs_seg = shobj_mlp_module.forward({cat_local_view}).toTensor().to(_libtorch_device);
        gs_contri = contri_mlp_module.forward({cat_local_view}).toTensor().to(_libtorch_device);
      } else {
        ...
        gs_seg = shobj_mlp_module.forward({cat_local_view_wodist}).toTensor().to(_libtorch_device);
        gs_contri = contri_mlp_module.forward({cat_local_view_wodist}).toTensor().to(_libtorch_device);
      }
    } else {
      if (_add_color_dist) {
        gs_color = color_mlp_module.forward({cat_local_view}).toTensor().to(_libtorch_device);
    ...
    gs_seg = gs_seg.reshape({M * 10, 15});
    gs_contri = gs_contri.reshape({M * 10, 1});

    gs_scale = gs_scale.unsqueeze(0).repeat({10, 1, 1}).transpose(0, 1).reshape({M * 10, 3});

    gs_pos = gs_pos.reshape({M * 10, 3});

    ak_contri = ak_contri.unsqueeze(0).repeat({10, 1, 1}).transpose(0, 1).reshape({M * 10, 1});
    ak_seg = ak_seg.unsqueeze(0).repeat({10, 1, 1}).transpose(0, 1).reshape({M * 10, 15});

    torch::Tensor all =
        torch::cat({gs_pos, gs_scale, gs_color, scale_rot, neural_opacity, ak_contri, ak_seg, gs_contri, gs_seg}, -1)
            .to(_libtorch_device);
    ;
    ...
    gs_contri = torch::sigmoid(all.narrow(-1, 33, 1)).mul(all.narrow(-1, 17, 1));
    gs_seg = torch::sigmoid(all.narrow(-1, 34, 15)).mul(all.narrow(-1, 18, 15));

    gs_seg = gs_seg.unsqueeze(1);
    ...
    if (currModal == "depth") {
      CUDA_SAFE_CALL(cudaMemcpy(
          fallback_depthout_bytes.data(),
          fallbackDepthOutBufferCuda,
          fallback_depthout_bytes.size(),
      ...
      CUDA_SAFE_CALL(cudaMemcpy(
          fallback_normalout_bytes.data(),
          fallbackNormalOutBufferCuda,
          fallback_normalout_bytes.size(),
          cudaMemcpyDeviceToHost));
      ...
      CUDA_SAFE_CALL(cudaMemcpy(
          fallback_segout_bytes.data(),
          fallbackSegOutBufferCuda,
          fallback_segout_bytes.size(),
          cudaMemcpyDeviceToHost));
      ...
      CUDA_SAFE_CALL(cudaMemcpy(
          fallback_knnmapout_bytes.data(),
          fallbackKnnmapOutBufferCuda,
          fallback_knnmapout_bytes.size(),
          cudaMemcpyDeviceToHost));
      ...
      CUDA_SAFE_CALL(cudaMemcpy(
          fallback_rgb_bytes.data(), fallbackRGBBufferCuda, fallback_rgb_bytes.size(), cudaMemcpyDeviceToHost));
