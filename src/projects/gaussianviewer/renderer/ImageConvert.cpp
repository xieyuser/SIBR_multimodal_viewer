...
torch::Tensor sibr::vis_normal(const torch::Tensor &normal_tensor)
{
    ...
    auto normal = normal_tensor.detach();
    
    auto mask1 = torch::isnan(normal);
    normal = normal.masked_fill(mask1, 0.0);

	auto mask2 = torch::isinf(normal);
    normal = normal.masked_fill(mask2, 0.0);
    
    normal = normal.clamp(-1.0, 1.0).add(1.0).div(2.0);
